...
export function diffCatalogs(
  ...
  const low_confidence: CatalogDiff["low_confidence"] = [];
    ...
    if (fieldsChanged.length > 0 || propertyChanges.length > 0) {
      modified.push({
        ...
        previous_description:
          fieldsChanged.includes("description") ? baseEvent.description : undefined,
        confidence: headEvent.confidence,
        confidence_changed: baseEvent.confidence !== headEvent.confidence,
        previous_confidence:
          baseEvent.confidence !== headEvent.confidence ? baseEvent.confidence : undefined,
...
function diffEventFields(base: CatalogEvent, head: CatalogEvent): string[] {
...
function diffProperties(
  baseProps: CatalogEvent["properties"],
  headProps: CatalogEvent["properties"]
): PropertyChange[] {
