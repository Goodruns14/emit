...
export class DestinationMcpClient {
  ...
  /**
   * List the tools the destination MCP exposes. Returns the subset of fields
   * that `emit enrich` cares about — name, description, and inputSchema —
   * so prompts can be built without leaking SDK internals.
   */
  async listTools(): Promise<
    { name: string; description?: string; inputSchema?: unknown }[]
  > {
    if (!this.client) {
      throw new Error("DestinationMcpClient.listTools called before connect()");
    }
    const result = (await this.client.listTools()) as {
